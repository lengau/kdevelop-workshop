    ...
    : QWidget(parent), m_plugin(plugin), m_refreshing(false)
{
    ...
    connect(m_projectCombo, QOverload<int>::of(&QComboBox::currentIndexChanged), this, &WorkshopTerminalToolView::onProjectChanged);
    ...
    connect(m_workshopCombo, QOverload<int>::of(&QComboBox::currentIndexChanged), this, &WorkshopTerminalToolView::updateWorkshopState);
    ...
    connect(KDevelop::ICore::self()->projectController(), &KDevelop::IProjectController::projectOpened,
            this, [this](KDevelop::IProject*) { QTimer::singleShot(100, this, &WorkshopTerminalToolView::refresh); });
...
void WorkshopTerminalToolView::onProjectChanged(int index)
{
    ...
    auto* thread = QThread::create([this, projectPath]() {
        ...
        QMetaObject::invokeMethod(this, [this, workshops]() {
            m_refreshing = false;
            ...
            if (m_workshopCombo->count() > 0) {
                if (firstReadyIndex != -1) {
                    m_workshopCombo->setCurrentIndex(firstReadyIndex);
                } else {
                    m_workshopCombo->setCurrentIndex(0);
                }
            }
...
void WorkshopTerminalToolView::updateWorkshopState()
{
    ...
    qWarning() << "=================> WorkshopTerminalToolView::updateWorkshopState index:" << index 
               << "name:" << workshopName 
               << "data type:" << dataVal.typeName() 
               << "data value:" << dataVal << "<=================";
    ...
    qWarning() << "=================> WorkshopTerminalToolView::updateWorkshopState status:" << status 
               << "needsLaunch:" << needsLaunch << "<=================";
    ...
    if (isReady) {
        ...
        if (needsLaunch) {
            ...
            showStatusMessage(QStringLiteral("<h3>Workshop <b>%1</b> is not launched</h3><p>Click 'Launch Workshop' above to build and start it.</p>").arg(workshopName));
            ...
            m_startBtn->setText(status.toLower() == QLatin1String("launching") ? QStringLiteral("Launching...") : QStringLiteral("Starting..."));
            showStatusMessage(QStringLiteral("<h3>Workshop <b>%1</b> is starting...</h3><p>Please wait while the container runs.</p>").arg(workshopName));
        } else if (status.toLower() == QLatin1String("stopping")) {
            m_startBtn->setEnabled(false);
            m_startBtn->setIcon(QIcon::fromTheme(QStringLiteral("view-refresh")));
            m_startBtn->setText(QStringLiteral("Stopping..."));
            showStatusMessage(QStringLiteral("<h3>Workshop <b>%1</b> is stopping...</h3><p>Please wait.</p>").arg(workshopName));
        } else {
            m_startBtn->setText(QStringLiteral("Start Workshop"));
            showStatusMessage(QStringLiteral("<h3>Workshop <b>%1</b> is stopped</h3><p>Click 'Start Workshop' above to run it.</p>").arg(workshopName));
        }
    }
}
...
void WorkshopTerminalToolView::connectToWorkshop(const QString& workshopName)
{
    ...
    const auto result = KPluginFactory::loadFactory(KPluginMetaData::findPluginById(
...
void WorkshopTerminalToolView::startWorkshop()
{
    ...
    if (action == QStringLiteral("stop")) {
        showStatusMessage(QStringLiteral("<h3>Stopping workshop <b>%1</b>...</h3><p>Connecting to API...</p>").arg(workshopName));
    } else {
        showStatusMessage(QStringLiteral("<h3>%1 workshop <b>%2</b>...</h3><p>Connecting to API...</p>")
    ...
    auto* thread = QThread::create([this, projectPath, workshopName, action]() {
        ...
        if (!projectId.isEmpty()) {
            ...
            errorMessage = QStringLiteral("Project is not registered in workshopd.");
        }

        if (success && !changeId.isEmpty()) {
            QJsonDocument waitResp = WorkshopApi::query(QStringLiteral("/v1/changes/%1/wait").arg(changeId));
            if (!waitResp.isEmpty()) {
        ...
        QMetaObject::invokeMethod(this, [this, success, errorMessage, workshopName]() {
            if (!success) {
                ...
                QMessageBox::critical(this, QStringLiteral("Action Failed"),
                    QStringLiteral("Failed to start/stop/launch workshop %1: %2").arg(workshopName).arg(errorMessage));
            } else {
